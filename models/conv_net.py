...
class ConvNet(nn.Module):
    def __init__(self, seq_len, in_channels, out_channels, kernel_size, stride, padding, num_classes,
                 ) -> None:
        ...
        self.conv_3 = nn.Conv1d(out_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding)
        self.LeakyReLU_3 = nn.LeakyReLU(0.01)
        
        # TODO: Assuming padding keeps things the right length
        l_out = out_channels
        fc_size = int(seq_len * l_out)
        self.fully_connected = nn.Linear(in_features=fc_size, out_features=num_classes)
...
def train_conv_net(trainloader, testloader):
    ...
    print((seq_len, in_channels, out_channels, kernel_size, stride, padding, num_classes))
    model = ConvNet(seq_len, in_channels, out_channels, kernel_size, stride, padding, num_classes)
